...
<head>
    ...
    <title><?php echo e($party['app_name']); ?> - Grafik & Statistik</title>
    ...
    <style>
        :root {
            ...
            --muted: #657181;
            --line: #d9dee5;
            --primary: <?php echo e($party['colors']['primary']); ?>;
            --primary-2: <?php echo e($party['colors']['primary_dark']); ?>;
            --red: <?php echo e($party['colors']['primary']); ?>;
            --red-soft: <?php echo e($party['colors']['primary_soft']); ?>;
            --blue-soft: #dbe8ff;
...
<aside class="admin-mobile-drawer admin-nav-sidebar flex flex-col border-r">
    <div class="p-5 flex items-center justify-between border-b border-slate-200">
        <div class="flex items-center gap-3">
            <div class="w-9 h-9 rounded-lg bg-[var(--red-soft)] border border-[var(--red)]/20 flex items-center justify-center overflow-hidden">
                <img src="<?php echo e(asset($party['assets']['logo'])); ?>" alt="<?php echo e($party['app_name']); ?> Logo" class="w-8 h-8 object-contain">
            </div>
            <div>
                <p class="text-lg font-extrabold text-[var(--primary)] leading-none">SIMAP</p>
                <p class="font-mono-data text-[10px] uppercase tracking-widest text-slate-500 mt-1"><?php echo e($roleLabel); ?></p>
            </div>
        </div>
    ...
    <nav class="flex-1 py-4 overflow-y-auto">
        <?php $__currentLoopData = $adminMenus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menu): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <a class="admin-nav-item <?php echo e($menu['key'] === 'chart' ? 'active' : ''); ?>" href="<?php echo e($menu['route']); ?>">
                <span class="material-symbols-outlined"><?php echo e($menu['icon']); ?></span>
...
<header class="fixed top-0 left-0 right-0 z-50 h-16 bg-white border-b border-slate-200 shadow-sm">
    <div class="h-full px-6 flex items-center justify-between gap-6">
        <div class="flex items-center gap-4 min-w-0">
            ...
            <a href="<?php echo e($homeRoute); ?>" class="w-10 h-10 rounded-lg bg-white border border-slate-200 flex items-center justify-center overflow-hidden shrink-0">
                <img src="<?php echo e(asset($party['assets']['logo'])); ?>" alt="<?php echo e($party['app_name']); ?>" class="w-8 h-8 object-contain">
            </a>
            <div class="min-w-0">
                <p class="text-[11px] uppercase tracking-[0.22em] text-slate-500 font-semibold">Sistem Informasi</p>
                <h1 class="text-lg font-extrabold text-[var(--primary)] truncate"><?php echo e($party['app_name']); ?></h1>
            </div>
        </div>
...
<main class="chart-shell pt-16 h-screen flex">
    <aside class="chart-nav admin-nav-sidebar hidden md:flex flex-col w-64 border-r h-full overflow-y-auto shrink-0 z-40">
        <div class="p-6 border-b border-slate-200">
            ...
            <span class="font-mono-data text-[10px] uppercase tracking-widest text-[var(--red)] bg-[var(--red-soft)] px-2 py-1 rounded"><?php echo e($roleLabel); ?></span>
        </div>
    ...
    <aside class="chart-filter w-[330px] bg-white text-slate-800 border-r border-slate-200 h-full overflow-y-auto flex flex-col p-6 shadow-xl z-40">
        <div class="mb-7">
            ...
            <input type="hidden" id="f-jenis" value="<?php echo e($defaultJenis); ?>">
            <div id="jenis-buttons" class="grid grid-cols-2 gap-2">
                <?php $__currentLoopData = \App\Models\RekapHeader::JENIS_LABELS; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if(in_array($key, $aktifJenis)): ?>
                        <button type="button"
                                data-jenis="<?php echo e($key); ?>"
                                onclick="selectJenis('<?php echo e($key); ?>')"
...
<script>
    ...
    window.SIMAP_CHART_CONFIG = {
        dataUrl: <?php echo json_encode(route('admin.rekap.chart.data'), 15, 512) ?>,
        geojsonVersion: <?php echo json_encode(max(
            file_exists(public_path('geojson/banyuwangi_kecamatan.geojson')) ? filemtime(public_path('geojson/banyuwangi_kecamatan.geojson')) : time(), file_exists(public_path('geojson/banyuwangi_desa_full.geojson')) ? filemtime(public_path('geojson/banyuwangi_desa_full.geojson')) : time()
        ), 512) ?>,
        kecamatans: <?php echo json_encode($kecamatans->map(fn($k) => ['id' => $k->id, 'nama' => $k->nama])->values(), 512) ?>,
        desas: <?php echo json_encode($kecamatans->flatMap(fn($k) => $k->desas->map(fn($d) => ['id' => $d->id, 'nama' => $d->nama, 'kecamatan_id' => $k->id]))->values()) ?>,
    };
</script>
<script src="<?php echo e(asset('js/rekap-admin-chart.js')); ?>?v=<?php echo e(file_exists(public_path('js/rekap-admin-chart.js')) ? filemtime(public_path('js/rekap-admin-chart.js')) : time()); ?>"></script>
</body>
</html>
<?php /**PATH C:\laragon\www\simap-partai-template\resources\views/rekap/admin/chart.blade.php ENDPATH**/ ?>