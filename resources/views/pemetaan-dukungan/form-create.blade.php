...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        ...
        <h2 class="font-display text-3xl tracking-wide dark:text-white text-gray-900 mt-2">TAMBAH PENDUKUNG</h2>
        <p class="text-xs dark:text-gray-500 text-gray-400">Silakan isi formulir pendataan pendukung di bawah ini dengan lengkap.</p>
    </div>
    ...
    <form method="POST" action="{{ route('pemetaan-dukungan.store') }}" enctype="multipart/form-data" 
        ...
        <div>
            ...
            <input type="text" name="nama" value="{{ old('nama') }}" placeholder="cth: Ahmad Fauzi" class="{{ $inputClass }}" required>
        </div>

        {{-- NIK --}}
        <div>
            <label class="{{ $labelClass }}">NIK (Nomor Induk Kependudukan) <span class="text-red-500">*</span></label>
            <input type="text" name="nik" value="{{ old('nik') }}" placeholder="16 digit angka KTP" pattern="[0-9]{16}" maxlength="16" class="{{ $inputClass }}" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="no_hp" value="{{ old('no_hp') }}" placeholder="cth: 08123456789" class="{{ $inputClass }}" required>
            </div>
            <div>
                <label class="{{ $labelClass }}">Dokumen KTP <span class="text-xs dark:text-gray-600 text-gray-400 font-normal">(opsional)</span></label>
        ...
        <div>
            ...
            <textarea name="alamat" rows="3" placeholder="cth: Jl. Raya No. 45 RT 02 RW 03" class="{{ $inputClass }}" required>{{ old('alamat') }}</textarea>
        </div>
        ...
        <div class="border-t dark:border-gray-700 border-gray-200 pt-6">
            ...
            <textarea name="catatan" rows="3" placeholder="Informasi pendukung tambahan..." class="{{ $inputClass }}">{{ old('catatan') }}</textarea>
        </div>
...
<script>
    ...
    // Restore dynamic values on validation fail
